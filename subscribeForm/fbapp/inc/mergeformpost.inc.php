...
function MergeFormPost ( ) {

	$dom = DOMDocument::loadHTML( FormPage::GetInstance()->source );
	...
	$cfg =& FormPage::GetInstance()->GetConfig( 'special' );

	if( $cfg && isset( $cfg[ 'maxnumerrors' ] ) )
		$display_max_error = $cfg[ 'maxnumerrors' ];
	...
	foreach( $xpath->query( '//input[@name]' ) as $e ) {
		...
		// checkboxes have a names like "check1[]", but only "check1" is present in $_POST
		if( isset( $_POST[ $tagname ] ) || isset( $_POST[ $tagname_stripped ] ) ) {

			switch( $e->getAttribute( 'type' ) ) {

			case 'radio':

				if( $e->getAttribute( 'value' ) == $_POST[ $tagname ] ) {
			...
			case 'checkbox':
				...
				if( isset( $_POST[ $tagname_stripped ]) &&
					is_array( $_POST[ $tagname_stripped ]) &&
					in_array( $e->getAttribute( 'value' ) , $_POST[ $tagname_stripped ] ) ) {
			...
			default:
				$e->setAttributeNode( new DOMAttr( 'value', $_POST[ $tagname ] ) );
			}
		}
		...
		if( isset( $_POST[ $name ] ) ) {
			...
			foreach( $e->getElementsByTagName( 'option' ) as $child ) {
				...
				if( $is_array ) {
					...
					if( in_array( $child->getAttribute( 'value' ), $_POST[ $name ] )  &&
					...
					if( $child->getAttribute( 'value' ) == $_POST[ $name ] &&
						! $child->hasAttribute( 'selected' ) ) {
	...
	foreach( $xpath->query( '//textarea[@name]' ) as $e ) {
		...
		if( isset( $_POST[ $name ] ) ) {
			...
			$e->appendChild( $dom->createTextNode ( $_POST[ $name ] ) );
		}
