 ...
 *
 * Errors must be reported to $this->page!
 *
...
class MailChimp extends PageExtension {
	...
	private function subscribe ( $mclist ) {
		...
		if( $this->result->error ) {
			...
			$this->setError( 'Error '. $this->result->code . ' while subscribing to the mailing list: ' . $this->result->error );
	...
	private function _prepareData ( $action, $config ) {
		...
		if( isset( $cfg['double_optin'] ) )			$this->data[ 'double_optin' ] = $cfg['double_optin'];
		if( isset( $cfg['update_existing'] ) )		$this->data[ 'update_existing' ] = $cfg['update_existing'];
		if( isset( $cfg['replace_interests'] ) )	$this->data[ 'replace_interests' ] = $cfg['replace_interests'];
		if( isset( $cfg['send_welcome'] ) )			$this->data[ 'send_welcome' ] = $cfg['send_welcome'];

		if( isset( $cfg[ 'email_type_field' ] ) &&
			($type = $this->getPost( $cfg[ 'email_type_field' ] )) &&
	...
	private function _contactMailChimp ( $method ) {
		...
		if( ! isset( $this->data[ 'email_address' ] ) ) {
			...
			$this->setError( 'Can\'t contact MailChimp if no Email address is specified. Check the error log.' );
		...
		if( ! $this->result ) {
			...
			$this->setError( 'Communication with MailChimp failed. Check the error log.' );
	...
	private function date ( $tag ) {
		...
		if( $date !== false ) 	$this->merges[ $tag[ 'tag' ] ] = date('Y-m-d', $date );		
	}
	...
	*/
	...
	private function imageurl ( $tag ) {
		...
		if( $rules === false || ! isset( $rules[ $fldnam ] ) ) {
			writeErrorLog( 'MailChimp plugin couldn\'t access rules or failed to locate:', $fldnam );
			return;
		}

		switch( $this->page->GetRulePropertyByName( $fldnam, 'fieldtype' ) ) {
	...
	private function _makePublicUrl ( $fieldname, $filename ) {
		...
		if( ! file_exists ( $this->page->getStorageFolder( 4 ) ) )
			mkdir( $this->page->getStorageFolder( 4 ) );

		// use the rules to find out where the file is
		if( $this->page->GetRulePropertyByName( $fieldname, 'files' ) == true ) {

			if( ! copy( $this->page->getStorageFolder( 1 ) . $filename, $this->page->getStorageFolder( 4 ) . $filename ) ) {
			...
			if( isset( $_FILES[ $fieldname ] ) &&
				...
				$filename = SaveUploadAsFile( $this->page->getStorageFolder( 4 ), $_FILES[ $fieldname ] );
				if( $filename == false ) {
		...
		return $url . $path . '/' . $this->page->GetFormName() . CC_FB_STORAGE_FOLDER . CC_FB_PUBLIC_DIRECTORY . $filename;
	}
