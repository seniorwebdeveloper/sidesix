...
class Channel_videos_ft extends EE_Fieldtype
{
	...
	public function display_field($data)
	{
		...
		$vData['layout'] = (isset($this->settings['cv_layout']) == TRUE) ? $this->settings['cv_layout'] : 'table';
		...
		$this->EE->channel_videos_helper->mcp_meta_parser('css', CHANNELVIDEOS_THEME_URL . 'channel_videos_pbf.css', 'cv-pbf');
		$this->EE->channel_videos_helper->mcp_meta_parser('css', CHANNELVIDEOS_THEME_URL . 'jquery.colorbox.css', 'jquery.colorbox');
		$this->EE->channel_videos_helper->mcp_meta_parser('js', CHANNELVIDEOS_THEME_URL . 'jquery.colorbox.js', 'jquery.colorbox', 'jquery');
		$this->EE->channel_videos_helper->mcp_meta_parser('js', CHANNELVIDEOS_THEME_URL . 'channel_videos_pbf.js', 'cv-pbf');
	...
	public function post_save($data)
	{
		...
		if ($query->num_rows() > 0)
		{
			...
			foreach ($data['videos'] as $order => $video)
			{
				...
				{
					$video = unserialize(base64_decode($video['data']));
