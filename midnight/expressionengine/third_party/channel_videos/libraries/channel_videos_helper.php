...
class Channel_videos_helper
{
	...
	public function get_router_url($type='url')
	{
		...
		if (isset($this->EE->session->cache['Channel_Videos']['Router_Url']['ACT_ID']) == FALSE)
		{
			$this->EE->db->select('action_id');
			$this->EE->db->where('class', 'Channel_videos');
			$this->EE->db->where('method', 'channel_videos_router');
			$query = $this->EE->db->get('actions');
			$ACT_ID = $query->row('action_id');
		}
		else $ACT_ID = $this->EE->session->cache['Channel_Videos']['Router_Url']['ACT_ID'];
		...
		if ($type == 'url')
		{
			...
			if (isset($this->EE->session->cache['Channel_Videos']['Router_Url']['URL']) == TRUE) return $this->EE->session->cache['Channel_Videos']['Router_Url']['URL'];
			$this->EE->session->cache['Channel_Videos']['Router_Url']['URL'] = $ajax_url;
			return $this->EE->session->cache['Channel_Videos']['Router_Url']['URL'];
		}
