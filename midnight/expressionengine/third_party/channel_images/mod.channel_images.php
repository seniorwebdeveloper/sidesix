...
class Channel_images
{
	...
	public function grouped_images($legacy=FALSE)
	{
		...
		foreach ($categories as $cat => $images)
		{
			...
			foreach ($images as $count => $image)
			{
				...
				$temp = $this->EE->channel_images_model->parse_size_vars($temp, $settings, $image);
	...
	public function prev_next_image($which='next')
	{
		...
		$this->EE->TMPL->tagdata = $this->EE->channel_images_model->parse_size_vars($this->EE->TMPL->tagdata, $settings, $image);
	...
	public function simple_image_url()
	{
		...
		if ($temp_dir == 'yes')
		{
			...
			if (! $fh = fopen(APPPATH.'cache/channel_images/field_'.$field_id.'/'.$dir.'/'.$file, 'rb'))
			{
				exit();
			}
