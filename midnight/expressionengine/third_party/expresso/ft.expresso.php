...
class Expresso_ft extends EE_Fieldtype {
	...
	function replace_tag($data, $params = array())
	{
		...
		foreach ($params as $key => $val) 
		{
			if ($key == 'paragraph_limit')
			{
				...
				{
					$pos = $this->_strnposr($data, '<p', ($val + 1));
	...
	/**
	  *  Better Workflow - check to see if we are loading a draft into the publish view
	  */
	private function _check_draft($data)
	{
		if (isset($this->EE->session->cache['ep_better_workflow']['is_draft']) AND $this->EE->session->cache['ep_better_workflow']['is_draft']) 
		{
